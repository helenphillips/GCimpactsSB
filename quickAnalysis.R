...
dataDir <- "C:/Users/nelli/Documents/WORK/GCimpactsSB/Data/September2021"

dat <- read.csv(file.path(dataDir, "processed", "alldata.csv"))
names(dat)[which(names(dat) == "�..ID")] <- "ID"
### REMOVE COLS ------------------------------
...
neededCols <- c("ID","Case_ID","CaseDescription", "System" ,  
               ...
               "TaxaGroup","TaxaBodysize", "GCDType", "ChangeType", "driver")
...
dat$GCD <- as.factor(dat$driver)


## PUT NS TO INTEGERS
dat$Control_N <- as.numeric(dat$Control_N) ## there is an N of 20000.....
dat$Treatment_N <- as.numeric(dat$Treatment_N) ## there is an N of 20000.....
...
frag_dat <- lrr_dat[lrr_dat$GCD == "HabitatLoss",]
frag.mod<-rma.mv(
  yi=LRR,
  V=LRR_var, 
  mods=~GCDType - 1,
...
y<-summary(frag.mod)$b
ci_l<-summary(frag.mod)$ci.lb
ci_h<-summary(frag.mod)$ci.ub

fg1<-data.frame(cbind(y,ci_l,ci_h))
colnames(fg1)[1]<-"y"
colnames(fg1)[2]<-"ci_l"
colnames(fg1)[3]<-"ci_h"
fg1$GCD<-c("Corridors/Connectivity","Edge effects", "Habitat amount",
           "Isolation")
fg1$GCD<-as.factor(fg1$GCD)
